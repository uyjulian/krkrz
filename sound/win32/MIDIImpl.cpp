...
public:
	...
	void Execute(void) // "Execute" override
	{
		while(!GetTerminated())
		{
			...
			if(sleepcount == buffercount) SetSuspend(); // all buffers are sleeping
...
void tTVPSMFTrack::Rewind(void)
{
	...
	Ended = false;
	PrevEvent = 0;
	const tjs_uint8 *pb = (const tjs_uint8 *)Position;
	Count= TVPGetSMFValue(pb);
}
//---------------------------------------------------------------------------
void tTVPSMFTrack::ProcessEvent()
{
	...
	else
	{
		...
		{
			const tjs_uint8 *pb = (const tjs_uint8 *)Position;
			tjs_uint32 dlen = TVPGetSMFValue(pb);
...
//---------------------------------------------------------------------------
tTJSNI_MIDISoundBuffer::tTJSNI_MIDISoundBuffer() : EventQueue(this,&tTJSNI_MIDISoundBuffer::UtilWndProc)
{
	Playing = false;
	Loaded = false;
	Volume =  100000;
...
#else
void tTJSNI_MIDISoundBuffer::UtilWndProc( NativeEvent& ev )
{
	// called by OS, caused by posting of status message from playing thread.
	if(ev.Message == WM_USER+1)
	{
...
bool tTJSNI_MIDISoundBuffer::OnTimer()
{
	...
	if(p==0)
	{
		...
		else
		{
			...
			EventQueue.PostEvent( NativeEvent(WM_USER+1) );
		}
		return false;
	}
...
void tTJSNI_MIDISoundBuffer::Open(const ttstr &name)
{
	...
	try
	{
		...
		const tjs_uint8 *pb = (const tjs_uint8 *)buf;
...
bool tTJSNI_MIDISoundBuffer::StartPlay()
{
	...
	if(TVPMIDIThread) TVPMIDIThread->ResetSuspend();
