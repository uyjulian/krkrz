...
INCFLAGS += -I. -Ibase -Ibase/win32 -Ienviron -Ienviron/win32 -Iextension -Iexternal -Iexternal/freetype/include -Iexternal/freetype/src -Iexternal/freetype/devel -Iexternal/zlib -Imovie/win32 -Imsg -Imsg/win32 -Iplatform/win32 -Isound -Isound/win32 -Itjs2 -Iutils -Iutils/win32 -Ivcproj -Ivisual -Ivisual/IA32 -Ivisual/gl -Ivisual/win32
...
VISUAL_WIN32_SOURCES += visual/win32/BitmapBitsAlloc.cpp visual/win32/BitmapInfomation.cpp visual/win32/DInputMgn.cpp visual/win32/GraphicsLoaderImpl.cpp visual/win32/LayerImpl.cpp visual/win32/NativeFreeTypeFace.cpp visual/win32/TVPScreen.cpp visual/win32/TVPSysFont.cpp visual/win32/VSyncTimingThread.cpp visual/win32/VideoOvlImpl.cpp visual/win32/WindowImpl.cpp
...
SOURCES := $(BASE_SOURCES) $(ENVIRON_SOURCES) $(EXTENSION_SOURCES) $(MOVIE_SOURCES) $(MSG_SOURCES) $(SOUND_SOURCES) $(TJS2_SOURCES) $(UTILS_SOURCES) $(VISUAL_SOURCES) $(VISUAL_WIN32_SOURCES) $(LIBZ_SOURCES) $(LIBFREETYPE_SOURCES)
