...
ENVIRON_SOURCES += environ/TouchPoint.cpp environ/win32/Application.cpp environ/win32/CompatibleNativeFuncs.cpp environ/win32/DetectCPU.cpp environ/win32/EmergencyExit.cpp environ/win32/MouseCursor.cpp environ/win32/SystemControl.cpp environ/win32/TVPWindow.cpp environ/win32/WindowFormUnit.cpp environ/win32/WindowsUtil.cpp
EXTENSION_SOURCES += extension/Extension.cpp
MOVIE_SOURCES += movie/win32/TVPVideoOverlay.cpp
MSG_SOURCES += vcproj/tvpwin32.rc msg/MsgIntf.cpp msg/win32/MsgImpl.cpp
...
VISUAL_SOURCES += visual/BitmapIntf.cpp visual/BitmapLayerTreeOwner.cpp visual/ComplexRect.cpp visual/DrawDevice.cpp visual/GraphicsLoadThread.cpp visual/GraphicsLoaderIntf.cpp visual/IA32/detect_cpu.cpp visual/ImageFunction.cpp visual/LayerBitmapImpl.cpp visual/LayerBitmapIntf.cpp visual/LayerIntf.cpp visual/LayerManager.cpp visual/LayerTreeOwnerImpl.cpp visual/NullDrawDevice.cpp visual/RectItf.cpp visual/TransIntf.cpp visual/VideoOvlIntf.cpp visual/WindowIntf.cpp visual/gl/ResampleImage.cpp visual/gl/WeightFunctor.cpp visual/tvpgl.c
VISUAL_WIN32_SOURCES += visual/win32/BitmapBitsAlloc.cpp visual/win32/BitmapInfomation.cpp visual/win32/DInputMgn.cpp visual/win32/GraphicsLoaderImpl.cpp visual/win32/LayerImpl.cpp visual/win32/TVPScreen.cpp visual/win32/VSyncTimingThread.cpp visual/win32/VideoOvlImpl.cpp visual/win32/WindowImpl.cpp
...
SOURCES := $(BASE_SOURCES) $(ENVIRON_SOURCES) $(EXTENSION_SOURCES) $(MOVIE_SOURCES) $(MSG_SOURCES) $(SOUND_SOURCES) $(TJS2_SOURCES) $(UTILS_SOURCES) $(VISUAL_SOURCES) $(VISUAL_WIN32_SOURCES) $(LIBZ_SOURCES)
